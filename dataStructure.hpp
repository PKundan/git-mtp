                   ...
                   std::vector<Edge> &boundaryEdges)
{
    ...
    for (size_t i = 0; i < cells.size(); ++i)
    {
        ...
        for (size_t k = 0; k < cellNodes.size() - 1; k++)
        {
            ...
            if (includesEdge(edg, boundaryEdges))
            {
                // edg.boundaryFlag = true;
                if (isIncludedEdgeEqual(edg, boundaryEdges))
                {
                    int indx = findEdgeIndex(edg, boundaryEdges);
                    boundaryEdges[indx].left_cell_index = i;
                    cells[i].boundaryFlag = true;
                }
            }
            else if (edgeIndx > 1)
            {
                if (includesEdge(edg, interiorEdges))
                {
                    edg.nodeIndices = vectorReverse(edg.nodeIndices);
                    int indx = findEdgeIndex(edg, interiorEdges);
                    interiorEdges[indx].right_cell_index = i;
                }
                else
                {
                    edg.index = edgeIndx;
...
void createBoundaryEdges(std::vector<Node>& nodes,
    ...
    for(const auto& edge : boundaryEdgesPrimal){
	    ...
	    int iMid = k; ++k;   // increment node index
    ...
    for(int iCell=0; iCell< cellsPrimal.size(); ++iCell){
        ...
        for(int i=0; i< iCellNodes.size()-1; ++i){
            ...
            if(includesEdge(edg, edgesPrimal)){
                ...
                edg.index = edgeIndexPrimal; edgeIndexPrimal+=1;
                edgesPrimal.push_back(edg);
                Node mid = edg.center(nodes);
                mid.flag = 2;
                nodes.push_back(mid);
                iEdgCenters[i] = nodeIndex; ++nodeIndex;
            }
        } 
        for(int k=0; k!=iEdgCenters.size(); ++k){
            ...
            edge.index = edgeIndex; ++edgeIndex;
